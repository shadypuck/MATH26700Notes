...
\begin{itemize}
    \item \marginnote{9/27:}HW will be due every week on Wednesday or thereabouts.
    \begin{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{proof}
            ...
            %     \end{bmatrix}
            % \end{equation*}
            % In particular, since permutation matrices (such that of $(12)$) are unitary, $\lambda=1$. Thus, $(12)\cdot x=x$

            Suppose for the sake of contradiction that $x=(x_1,\dots,x_n)$ is a nonzero vector that is fixed under the standard representation, that is, $\sigma x=\lambda_\sigma x$ for all $\sigma\in S_n$. Note that $n\geq 3$. (If $n=2$, the definition of the standard representation renders it equal to the alternating representation; if $n\in\{0,1\}$, its definition renders it equal to the zero representation.) Then for $i=2,\dots,n$, we have
            \begin{equation*}
                (x_i,x_2,\dots,x_{i-1},x_1,x_{i+1},\dots,x_n) = (1i)\cdot x
                = \lambda x
                = (\lambda x_1,\dots,\lambda x_n)
            \end{equation*}
            Let $j\notin\{1,i\}$; recall that such a $j$ is guaranteed to exist since $n\geq 3$. It follows since $x_j=\lambda x_j$ that $\lambda=1$. Thus, it follows that $x_i=\lambda x_1=x_1$. Therefore, $x=(x_1,\dots,x_1)$. But since $x_1+\cdots+x_n=0$ for any $x$ in the standard representation, we have that
            \begin{align*}
                0 &= x_1+\cdots+x_n = x_1+\cdots+x_1 = n\cdot x_1\\
                0 &= x_1
            \end{align*}
            It follows that $x=(0,\dots,0)=0$, contradicting the hypothesis that $x$ is nonzero.
        \end{proof}
    \end{itemize}
